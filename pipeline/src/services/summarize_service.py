...
class SummarizeService:
    ...
    def summarize_articles(self, limit: int = 50) -> Dict[str, Any]:
        """要約されていない記事を処理"""
        try:
            # 要約されていない記事を取得
    ...
    def summarize_specific_articles(self, article_ids: List[str]) -> Dict[str, Any]:
        """特定の記事を要約処理"""
        processed = 0
        errors = 0
        ...
        for article_id in article_ids:
            try:
                # 記事詳細を取得（実装は省略、必要に応じて追加）
                # article = self.db.get_article_by_id(article_id)
                # if not article:
                #     continue
                
                # 要約処理を実行
                # summary, labels = self.llm.generate_summary_and_labels(article["content"])
